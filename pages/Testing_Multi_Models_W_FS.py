...
def test_model(models, test_data_raw, outcomes, train_data_raw=None, cutoff_index='youden'):
    ...
    for outcome in outcomes:
        ...
        try:
            results_dictonary[outcome]['feature importance'] = feature_importance_df_sorted
        except:
...
if uploaded_models is not None and uploaded_test_set is not None and len(selected_outcomes)!=0:
    # button to test the models
    if st.button('Test the models'):

        # call the function to test models
        results_dictonary, outcome_dic, feature_importance_dic = test_model(models_dic, test_set, selected_outcomes, train_data_raw=train_set, cutoff_index='youden')
